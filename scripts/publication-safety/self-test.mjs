...
{
  const repo = makeRepo();
  stageFile(repo, 'api/test/.env.test.example', 'SAFE_EXAMPLE=example\n');
  expectPass('allows test env examples', 'node', [scanScript], repo);
}

{
  const repo = makeRepo();
  stageFile(repo, 'packages/data-schemas/src/models/user.ts', 'export const modelName = "user";\n');
  expectPass('allows source model directories', 'node', [scanScript], repo);
}
...
{
  const repo = makeRepo();
  stageFile(repo, 'README.md', '# Empty summary marker write test\n');
  expectFail(
    'empty marker summary is rejected on write',
    'node',
    [
      markerScript,
      '--write',
...
{
  const repo = makeRepo();
  stageFile(repo, 'README.md', '# Empty summary marker check test\n');
  const hash = must('node', [markerScript, '--hash'], repo).stdout.trim();
  mkdirSync(path.join(repo, '.git', 'publication-safety'), { recursive: true });
  writeFileSync(
    path.join(repo, '.git', 'publication-safety', `${hash}.json`),
    `${JSON.stringify({
      hash,
      verdict: 'pass',
      reviewers: ['secrets', 'internal', 'license'],
      summary: ' ',
    }, null, 2)}\n`,
  );
  expectFail('empty marker summary is rejected on check', 'node', [markerScript, '--check'], repo);
}
